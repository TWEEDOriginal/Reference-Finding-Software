...
import PyPDF2
import re
def pdf_references_getter(pdf_file, reference_type):
    square_bracket = r'''
                ([^\[\.]+)
                (\[\d+\])
                ([\w\s,';"!:()#%/$+-]*)
    ...
    paragraph_num = 0
    lsd = re.compile(r'([^0-9A-Zl\)])\. ')
    with open(str(pdf_file), 'rb') as pdfFileObj:
        pdfReader = PyPDF2.PdfFileReader(pdfFileObj)
        if not pdfReader.isEncrypted: #to check if it is encrypted
            title = pdfReader.documentInfo.title
            pages = pdfReader.numPages
            for i in range(pages):    
                pageObj = pdfReader.getPage(i)
                text = pageObj.extractText()
                text = text.replace('\n', '').replace('\r','')
                lsd_trip = re.sub(lsd, r"\1. \n", text)
                line = lsd_trip.strip()   
                if line != "":
